...
export function calculatePosition(date: dayjs.Dayjs, dateRange: dayjs.Dayjs[], layout) {
  //hours column width is 50 pixel
  const realWidth = layout.width - 50;
  let a = 0
  for (let i = 0; i < dateRange.length; i++) {
    if (dateRange[i].day() == date.day())
      a = i
  }
  const leftIndex = a
  const relativeX = (leftIndex * realWidth) / 7
  const x = 50 + relativeX
  const y = (1000 / 24) * date.hour()
  return { x, y }
}

export function getWidthOfAllDayInPercent(start: Date, end: Date) {
  const diffInMiliSeconds = dayjs(end).diff(dayjs(start))
  const diffInDays = diffInMiliSeconds / 86400000
  return diffInDays * 97
}